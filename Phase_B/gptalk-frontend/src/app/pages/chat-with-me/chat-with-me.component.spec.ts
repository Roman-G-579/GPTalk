...
describe('ChatWithMeComponent', () => {
	...
	it('should add user message to conversation and call chat service', () => {
		...
		expect(mockChatWithMeService.chat).toHaveBeenCalledWith('Hello', Language.English, [
	...
	it('should handle grading and open dialog', () => {
		...
		const confirmDialogMock = jest
			.spyOn(chatComp['confirmationService'], 'confirm')
		...
		expect(mockChatWithMeService.grade).toHaveBeenCalledWith(Language.English, [
			{ role: 'user', content: 'Hello' },
		]);
		...
		expect(confirmDialogMock).toHaveBeenCalledWith({
			...
			message: expect.stringContaining(
				`Your grade is: 80! Good Job! <br><br> Exp reward: 160 <br><br> Feedback: mock feedback`,
			),
	...
	it('should open dialog with correct grade and navigate home on reject', () => {
		...
		expect(confirmDialogMock).toHaveBeenCalledWith({
			header: 'Session Grade',
			message: expect.stringContaining(
				`Your grade is: 60! Better luck next time! <br><br> Exp reward: 120 <br><br> Feedback: mock feedback`,
			),
			accept: expect.any(Function),
			reject: expect.any(Function),
		});
	...
	it('should set empty language and clear conversation on accept in dialog', () => {
		...
		// Mocks the acceptance of a confirm dialog
		jest.spyOn(chatComp['confirmationService'], 'confirm').mockImplementation(((
